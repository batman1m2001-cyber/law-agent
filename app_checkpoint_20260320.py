...
def _article_num(chunk) -> int:
    m = re.search(r"\d+", chunk.article_id)
...
def scan_file(file):
    ...
    try:
        ...
        nums = [_article_num(c) for c in chunks]
        min_a, max_a = min(nums), max(nums)
        meta = extract_metadata(text)
        return (
            f"<div style='padding:8px;background:#e3f2fd;border-radius:6px;font-size:13px'>"
            f"<b>{meta.so_van_ban}</b> — {len(chunks)} điều "
            f"(Điều <b>{min_a}</b> → Điều <b>{max_a}</b>). "
            f"Để xử lý tất cả, để trống ô 'Từ điều' và 'Đến điều'.</div>"
        )
...
def process_document(file, from_article, to_article):
    ...
    # Filter by article range if specified
    from_n = int(from_article) if from_article else None
    to_n = int(to_article) if to_article else None
    if from_n is not None or to_n is not None:
        chunks = [
            c for c in chunks
            if (from_n is None or _article_num(c) >= from_n)
            and (to_n is None or _article_num(c) <= to_n)
        ]
        if not chunks:
            yield (
                f"<b style='color:orange'>Không tìm thấy điều nào trong khoảng "
                f"Điều {from_n} — Điều {to_n}.</b>"
            ), pd.DataFrame(), None, [], None
            return

    n = len(chunks)
    range_label = f" (Điều {from_n}–{to_n})" if (from_n or to_n) else ""
    header = f"<b>{meta.so_van_ban}</b> — {n} điều{range_label}"
